...
const STRESS_MAPPING_A = { Low: 2.29, Moderate: 4.8, High: 7.42 };
...
// Model B input ranges (clamp to training domain): key -> [lo, hi]
const B_RANGES = {
  study_hours: [5.0, 10.0, 7.48],
  sleep_hours: [5.0, 10.0, 7.5],
  eca_hours: [0.0, 4.0, 1.99],
  social_hours: [0.0, 6.0, 2.7],
...
function predictBurnout(inp) {
  ...
  const f = {
    study_hours_per_day: inp.study_hours,
    sleep_hours: inp.sleep_hours,
    exam_pressure: inp.exam_pressure,
    stress_level: stress,
    financial_stress: inp.financial_stress,
    social_support: inp.social_support,
    anxiety_score: inp.anxiety_score,
    depression_score: inp.depression_score,
    family_expectation: inp.family_expectation,
    physical_activity: inp.physical_hours,
  };
  // engineer_features_a (src/features.py) - order MUST match training
  const vec = [
    f.study_hours_per_day,
    f.sleep_hours,
    f.exam_pressure,
    f.stress_level,
    f.financial_stress,
    ...
    (f.exam_pressure + f.financial_stress + f.family_expectation) / 3 -
      f.social_support, // pressure_support_gap
  ...
  return {
    classId,
    className: BURNOUT_CLASSES[classId],
    probabilities: proba,
    score,
  };
}
...
function predictGpa(inp) {
  const clamp = (k) => {
    const [lo, hi] = B_RANGES[k];
    return Math.max(lo, Math.min(hi, inp[k]));
  };
  const vec = [
    clamp("study_hours"),
    clamp("eca_hours"),
...
function burnoutGauge(score, className, classId) {
  ...
  const cx = 150,
    ...
    r = 110;
  const col =
    [COLORS.healthy, COLORS.mild, COLORS.burnout][classId] || COLORS.text;
  const ang = (v) => 270 + (v / 100) * 180;
  // pt() returns "x,y" string for a given score value
  const pt = (v) => {
    const [x, y] = polar(cx, cy, r, ang(v));
...
function gpaRing(gpa, label) {
  const r = 54,
    ...
    off = c * (1 - gpa / 4);
  const col =
    gpa >= 3.0 ? COLORS.healthy : gpa >= 2.5 ? COLORS.mild : COLORS.burnout;
  return `
...
function radarChart(inp) {
  ...
  const grid = [2, 4, 6, 8, 10]
    .map((g) => {
      const pts = cats
        .map((_, i) => polar(cx, cy, (R * g) / 10, (i * 360) / n).join(","))
        .join(" ");
  ...
  const dataPts = vals
    .map((v, i) =>
      polar(cx, cy, (R * Math.max(0, v)) / 10, (i * 360) / n).join(","),
    )
    .join(" ");
  const labels = cats
    .map((c, i) => {
  ...
  <svg viewBox="0 0 340 340" width="100%" style="max-width:380px">
    ...
    ${vals
      .map((v, i) => {
        const [x, y] = polar(cx, cy, (R * Math.max(0, v)) / 10, (i * 360) / n);
...
const state = {
  ...
  exam_pressure_rating: "High",
  family_expectation_rating: "Moderate",
  financial_stress_rating: "Low",
  social_support_rating: "High",
  anxiety_score_rating: "Moderate",
  depression_score_rating: "Very Low",
};
...
function runResults() {
  const inp = {
    ...
    eca_hours: state.eca_hours,
    social_hours: state.social_hours,
  ...
  document.getElementById("gauge").innerHTML = burnoutGauge(
    burnout.score,
    burnout.className,
    burnout.classId,
  );
  ...
  document.getElementById("probbars").innerHTML = probabilityBars(
    burnout.probabilities,
  );
  const col = { 0: COLORS.healthy, 1: COLORS.mild, 2: COLORS.burnout }[
    burnout.classId
  ];
  ...
  document.getElementById("latency").textContent =
    `Inference ran locally in ${ms} ms — no server round-trip.`;
...
async function submitFeedback() {
  ...
  if (!rating) {
    status.textContent = "Please pick a star rating.";
    status.style.color = COLORS.burnout;
    return;
  }
  ...
  try {
    const res = await fetch("/api/feedback", {
      ...
      body: JSON.stringify({
        ...r.inp,
        burnout_gauge_score: r.burnout.score,
        burnout_class: r.burnout.className,
        predicted_gpa: r.gpa.gpa,
        rating,
        comment,
      }),
    });
    ...
    status.style.color = COLORS.healthy;
  } catch (e) {
    status.textContent = "Could not submit feedback (offline or not deployed).";
    status.style.color = COLORS.mild;
  }
}
...
function init() {
  ...
  document
    ...
    .addEventListener("click", () => show("step2"));
  document
    .getElementById("backTo1")
    .addEventListener("click", () => show("step1"));
  ...
  document.querySelectorAll("#fb-stars button").forEach((b, i) => {
    b.addEventListener("click", () => {
      state._fbRating = i + 1;
      document
        .querySelectorAll("#fb-stars button")
        .forEach((x, j) => x.classList.toggle("on", j <= i));
    });
  });
  document
    .getElementById("fb-submit")
