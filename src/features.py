...
def engineer_features_b(df):
    ...
    df["study_leisure_ratio"] = df["study_hours"] / (
        df["social_hours"] + df["physical_hours"] + 1
    )
    ...
    df["study_efficiency"] = df["study_hours"] / (
        df["study_hours"] + df["social_hours"] + df["physical_hours"] + 1
    )
